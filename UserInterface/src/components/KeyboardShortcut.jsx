...
function KeyPill({ keys }) {
  return (
    <div className="inline-flex items-center gap-2 bg-white dark:bg-[#0f0f0f] border-2 border-gray-100 dark:border-gray-800 px-2 py-1 rounded-lg shadow-sm text-xs sm:text-sm">
      {keys.map((k, i) => (
        <span
          key={i}
          className="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 rounded-md font-medium text-gray-800 dark:text-gray-200 border-2 border-gray-200 dark:border-gray-700 whitespace-nowrap"
        >
          {k}
        </span>
      ))}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
        <header className="mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <h1 className="text-4xl font-semibold tracking-tight text-black dark:text-white mb-4">
                Keyboard Shortcuts
              </h1>
            ...
            <div className="flex items-center gap-3">
              <div className="relative w-full sm:w-64">
                ...
                <input
                  ...
                  className="w-full rounded-lg border-4 border-gray-100 dark:border-gray-800 bg-white dark:bg-[#0f0f0f] text-black dark:text-white text-sm px-3 py-2 placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-blue-300 focus:outline-none shadow-sm"
                />
                {query && (
                  <button
                    onClick={() => setQuery("")}
                    className="absolute right-1 top-1/2 -translate-y-1/2 px-2 py-1 text-xs text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200"
          ...
          <div className="mt-4 sm:flex sm:items-center sm:justify-between">
            ...
            <div className="mt-3 sm:mt-0 text-right text-xs text-gray-500 dark:text-gray-400">
              <span>Related: </span>
              <a
        ...
        <main className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <section className="lg:col-span-2 space-y-6">
            {filtered.length === 0 ? (
              <div className="rounded-lg bg-white dark:bg-[#0f0f0f] border-2 border-gray-100 dark:border-gray-800 p-6 text-center shadow-sm">
                <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">No shortcuts found</h3>
                <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">Try different keywords like "tab", "zoom", "find", or clear the search.</p>
              </div>
            ) : (
              filtered.map((group) => {
                ...
                return (
                  <article
                    key={group.id}
                    className="rounded-3xl bg-white dark:bg-[#0f0f0f] border-4 border-gray-100 dark:border-gray-800 p-5 pink"
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <h3 className="text-lg sm:text-xl font-semibold text-gray-800 dark:text-gray-200">{group.title}</h3>
                    ...
                    >
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {group.shortcuts.map((s, idx) => (
                          <div
                            key={idx}
                            className="flex items-center justify-between gap-3 p-3 bg-gray-50 dark:bg-gray-900 border-4 border-gray-100 dark:border-gray-800 rounded-xl"
                          >
                            <div className="flex-1">
                              <div className="text-sm font-medium text-gray-800 dark:text-gray-200">{s.desc}</div>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                ...
                                className="ml-2 px-2 py-1 rounded-md text-xs bg-white dark:bg-[#0f0f0f] text-gray-800 dark:text-gray-200 border-2 border-gray-100 dark:border-gray-800 hover:bg-gray-100 dark:hover:bg-gray-800 focus:outline-none"
                                title="Copy shortcut"
          ...
          <aside className="space-y-6">
            <div className="rounded-3xl bg-white dark:bg-[#0f0f0f] border-4 border-gray-100 dark:border-gray-800 p-4 shadow-sm">
              <h4 className="text-sm font-semibold text-gray-800 dark:text-gray-200">Legend</h4>
              <dl className="mt-3 text-sm text-gray-600 dark:text-gray-400 space-y-2">
                <div>
                  <dt className="font-medium text-gray-700 dark:text-gray-300">Key chips</dt>
                  <dd className="text-sm text-gray-500 dark:text-gray-400 mt-1">Each pill shows the exact keys to press in order — e.g., <span className="font-medium">Ctrl + T</span>.</dd>
                </div>
                <div>
                  <dt className="font-medium text-gray-700 dark:text-gray-300">Copy</dt>
                  <dd className="text-sm text-gray-500 dark:text-gray-400 mt-1">Quickly copy a shortcut description to clipboard for your docs.</dd>
                </div>
              </dl>
            </div>

            <div className="rounded-2xl bg-white dark:bg-[#0f0f0f] border-2 border-gray-100 dark:border-gray-800 p-4 shadow-sm">
              <h4 className="text-sm font-semibold text-gray-800 dark:text-gray-200">Safety & notes</h4>
            ...
            <div className="rounded-2xl bg-white dark:bg-[#0f0f0f] border-2 border-gray-100 dark:border-gray-800 p-4 shadow-sm">
              <h4 className="text-sm font-semibold text-gray-800 dark:text-gray-200">Coming soon</h4>
              <ul className="mt-3 text-sm text-gray-600 dark:text-gray-400 list-disc list-inside space-y-1">
