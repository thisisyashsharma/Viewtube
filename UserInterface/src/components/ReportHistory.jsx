      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6 mt-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Your reports for videos and their status.</p>
          </div>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <input
              ...
              className="flex-1 md:flex-none w-full md:w-64 p-2 border rounded-lg focus:ring-2 focus:ring-blue-500"
            />
        ...
        <div className="bg-white rounded-2xl shadow-sm border p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {loading && <div className="col-span-full text-center p-6 text-gray-500">Loading…</div>}

            {!loading && reports.length === 0 && (
              <div className="col-span-full text-center p-8 text-gray-500">No reports found.</div>
            )}

            {reports.map((r) => (
              <div key={r._id || r.id} className="flex flex-col rounded-lg border p-3 bg-white hover:shadow-sm transition">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <div className="font-semibold text-sm truncate max-w-xs">{r.videoTitle || 'Untitled video'}</div>
                        <div className="text-xs text-gray-400">Reported: {formatDistanceToNow(new Date(r.createdAt || r.date || Date.now()))} ago</div>
                      </div>
                    ...
                    <div className="mt-2 text-sm text-gray-600 truncate">Reason: {r.reason || 'N/A'}</div>

                    <div className="mt-3 flex items-center gap-2">
                      <button onClick={() => openDetail(r)} className="px-3 py-1 rounded-md bg-blue-600 text-white text-sm">View</button>
                      <a href={r.videoUrl || '#'} target="_blank" rel="noreferrer" className="px-3 py-1 rounded-md bg-gray-100 text-sm">video</a>
                      {r.status !== 'withdrawn' && r.status !== 'closed' && (
                        <button onClick={() => withdrawReport(r._id || r.id)} className="px-3 py-1 rounded-md bg-red-50 text-red-600 text-sm border">Withdraw</button>
                      )}
                    </div>
                  </div>
                </div>
          ...
          <div className="mt-6 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button disabled={page <= 1} onClick={() => setPage(p => Math.max(1, p - 1))} className="px-3 py-1 rounded border bg-white">Prev</button>
              <button disabled={page >= totalPages} onClick={() => setPage(p => Math.min(totalPages, p + 1))} className="px-3 py-1 rounded border bg-white">Next</button>
            </div>
          </div>
        </div>
        ...
        {selected && (
          <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
            ...
            <div className="relative w-full md:w-3/4 lg:w-1/2 bg-white rounded-t-xl md:rounded-xl p-4 md:p-6 shadow-xl overflow-y-auto max-h-[90vh]">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h2 className="text-lg font-semibold">Report details</h2>
                  <div className="text-sm text-gray-500 mt-1">Video: {selected.videoTitle}</div>
                </div>
                <div className="flex items-center gap-2">
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="md:col-span-2">
                  <div className="text-sm text-gray-700">Reported by you on {new Date(selected.createdAt).toLocaleString()}</div>
                  <div className="mt-3 text-sm text-gray-800">Reason: <span className="font-medium">{selected.reason}</span></div>

                  {selected.notes && (
                    <div className="mt-4">
                      <h3 className="font-medium">Your notes</h3>
                      <p className="text-sm text-gray-600 mt-1 whitespace-pre-wrap">{selected.notes}</p>
                    </div>
                  )}
                  ...
                  <div className="mt-6 flex gap-2">
                    {selected.status !== 'withdrawn' && selected.status !== 'closed' && (
                      <button onClick={() => withdrawReport(selected._id || selected.id)} className="px-4 py-2 bg-red-50 text-red-600 rounded">Withdraw report</button>
                    )}
                    <a href={selected.videoUrl || '#'} target="_blank" rel="noreferrer" className="px-4 py-2 bg-gray-100 rounded">video</a>
                  </div>
                </div>
              </div>
            </div>
